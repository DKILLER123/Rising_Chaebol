...
This repository is the production workspace for the 118-chapter English EPUB edition of
...
4. `worklog.md` — the complete audit trail; the newest entry is the chapter 114–118 production checkpoint.
...
epub/
  ...
  OEBPS/                    extracted/editable book tree
    text/                   118 chapters plus front matter
...
- 118 chapter XHTML files are present in `epub/OEBPS/text/`.
- The current package has 188 ZIP file entries, 118 chapter documents, 40 images, and 17 WOFF
  faces; its SHA-256 and final parity report are recorded in `worklog.md` and `reports/workspace_audit.json`.
- `extracted/` remains the immutable 113-chapter forensic snapshot from setup; current editorial
  source is under `epub/` and current deliverable is under `public/`.
- The 118-chapter package is the current deliverable; run the pre-package gates before any future
  rebuild.
